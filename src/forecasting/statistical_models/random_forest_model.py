...
- Much faster than recursive (32 models vs 2880 iterations) or 
...
class RandomForestForecaster:
    """
    Random Forest forecaster with direct multi-horizon approach.
    ...
    - MultiOutput: Training 2880 separate RF models
    """
    ...
    def __init__(self,
        ...
        self.models: Dict[str, Dict[int, RandomForestRegressor]] = {}
        self.feature_names: List[str] = []

    def fit(self, X_train: np.ndarray, y_train: np.ndarray, feature_names: list):
        """
        Fit Random Forest models for each feature at each key horizon.
        ...
        logger.info(f"Training {total_models} Random Forest models ({n_targets} features × {len(self.KEY_HORIZONS)} horizons)...")
        ...
        logger.info(f"Fitted {model_count} Random Forest models")
    ...
    def save(self, save_dir: str):
        ...
        for feat_name in self.feature_names:
            for horizon, model in self.models[feat_name].items():
                save_path = os.path.join(save_dir, f'rf_{feat_name}_h{horizon}.pkl')
                with open(save_path, 'wb') as f:
                    pickle.dump(model, f)

        logger.info(f"Saved Random Forest models to {save_dir}")
    ...
    def load(self, save_dir: str):
        ...
        for feat_name in self.feature_names:
            self.models[feat_name] = {}
            for horizon in self.KEY_HORIZONS:
                load_path = os.path.join(save_dir, f'rf_{feat_name}_h{horizon}.pkl')
                with open(load_path, 'rb') as f:
                    self.models[feat_name][horizon] = pickle.load(f)

        logger.info(f"Loaded Random Forest models from {save_dir}")


def run_random_forest_experiment(scenario: str = 'A', forecast_horizon: int = 2880):
    """
    Run complete Random Forest experiment for a scenario.
    ...
    """
    logger.info(f"\n{'='*80}")
    logger.info(f"Running Random Forest experiment - Scenario {scenario}")
    logger.info(f"{'='*80}\n")

    # Load data - handle small datasets gracefully
    ...
    # Save model
    save_dir = f"weights/random_forest_scenario_{scenario}"
    ...
    results = {
        ...
        'approach': 'direct_multi_horizon',
        'key_horizons': RandomForestForecaster.KEY_HORIZONS,
    ...
    results_path = f'results/random_forest_scenario_{scenario}_results.json'
    ...
    if test_results is not None:
        ...
        logger.info(f"\n{'='*80}")
        logger.info("SUMMARY - Validation Set Performance")
        logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    ...
    run_random_forest_experiment(scenario=scenario, forecast_horizon=2880)
