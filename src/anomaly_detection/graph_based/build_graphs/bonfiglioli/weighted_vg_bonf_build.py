...
# Build Weighted Visibility Graphs for Bonfiglioli Dataset with 'distance'
#compute degrees distributions of each graph
...
# Define target column keys to search for
...
for key in target_keys:
    ...
    if matching_cols:
        data_columns.extend(matching_cols)
        print(f"Found column(s) for '{key}': {matching_cols}")
    else:
        print(f"Warning: No column found matching '{key}'")
...
print(f"{'='*60}")
print(f"Selected columns ({len(data_columns)}): {data_columns}")

# Output directory
output_dir = Path('/home/projects/safe/outputs/networks/grafi/bonfiglioli_graphs/weighted')
...
degrees_dir = Path('/home/projects/safe/outputs/networks/grafi/bonfiglioli_graphs/weighted/degrees')
