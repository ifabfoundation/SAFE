...
# Plot Top 20 Oddball Nodes (Weighted - W_i ~ E_i^beta) on Original Time Series - Bonfiglioli
...
# Paths
oddball_dir = Path('/home/projects/safe/outputs/networks/oddball/bonfiglioli/oddball_scores/weighted_scores')
data_file = Path('/home/projects/safe/data/bonfiglioli/processed/ETR_946 - Prova 30_5 - 310 - Fatica 336h_1_1.csv')
output_dir = Path('/home/projects/safe/outputs/networks/oddball/bonfiglioli/oddball_vs_timeseries/heavyvicinity')
...
# Graph format: G_NVG_bonfi_gb1_tan_oil
# Column format: bonfi/gb1_tan_oil
def graph_to_column(graph_name):
    """
    Extract column name from graph name.
    G_NVG_bonfi_gb1_tan_oil -> bonfi/gb1_tan_oil
    """
    ...
    # Replace first underscore with slash: bonfi_gb1 -> bonfi/gb1
    parts = col.split('_', 2)  # Split into [bonfi, gb1, rest]
    if len(parts) >= 2 and parts[0] == 'bonfi':
        if len(parts) == 3:
            col = f"bonfi/{parts[1]}_{parts[2]}"
        else:
            col = f"bonfi/{parts[1]}"
...
print("\n" + "=" * 60)
print("PLOTTING TOP 20 ODDBALL NODES ON TIME SERIES")
print("=" * 60)
...
for oddball_file in oddball_files:
    ...
    try:
        ...
        ax.set_title(f'Top 40 Oddball Nodes - {graph_name}\n(weights: distance, oddball: heavyvicinity W_i ~ E_i^β)', fontsize=14, fontweight='bold')
