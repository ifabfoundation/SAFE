...
#   --tflite ../finale/qbench_sweep_20251027_165731/experiments/mt2000_b0p5/tflite/model_int8_full.tflite \
#   --csv /home/projects/safe/data/processed-datasets/processed_streaming_row_continuous.csv \
#   --start 0 --stm32 2000-05//outINT8.txt

# # Config 500/0.6
# python3 golden_compare_all.py \
#   --savedmodel ../export_cnn_lstm/savedmodel_unroll \
#   --tflite ../finale/qbench_sweep_20251027_165731/experiments/mt500_b0p6/tflite/model_int8_full.tflite \
#   --csv /home/projects/safe/data/processed-datasets/processed_streaming_row_continuous.csv \
#   --start 0 --stm32 500-06/outINT8.txt
...
def main():
    ...
    print(f"\nΔ TFLite vs Keras  →  MSE={mse_t:.6e}  RMSE={rmse_t:.6e}  MAE={mae_t:.6e}  MAX={maxe_t:.6e}")
    ...
    if args.stm32:
        ...
        print(f"Δ STM32 vs Keras   →  MSE={mse_s:.6e}  RMSE={rmse_s:.6e}  MAE={mae_s:.6e}  MAX={maxe_s:.6e}")

        # confronto diretto STM32 vs TFLite per test LSB
        max_diff_ts = float(np.max(np.abs(ystm - yt)))
        entro_lsb = (max_diff_ts <= (out_scale + 1e-8))
        print(f"[LSB] OUT_SCALE={out_scale:.9f} → 1 LSB")
        print(f"[CHECK] Δmax(STM32−TFLite)={max_diff_ts:.6e} → entro_LSB={entro_lsb}")
    ...
    summary = {
        ...
        "delta_tflite": {"mse": mse_t, "rmse": rmse_t, "mae": mae_t, "max": maxe_t},
    }
    if args.stm32:
        summary["delta_stm32"] = {"mse": mse_s, "rmse": rmse_s, "mae": mae_s, "max": maxe_s}
        summary["lsb"] = out_scale
        summary["delta_max_stm32_vs_tflite"] = max_diff_ts
        summary["entro_lsb"] = bool(entro_lsb)
    print("\nJSON_SUMMARY:", json.dumps(summary, ensure_ascii=False))

    # 8) CSV umano con 3 righe (Keras, TFLite, STM32) + nota LSB/entro_LSB
    if args.stm32:
        out_dir = Path(args.stm32).parent
        out_csv = out_dir / "compare_human.csv"

        keras_range = (float(np.min(yk)), float(np.max(yk)))
        tflite_range = (float(np.min(yt)), float(np.max(yt)))
        stm32_range = (float(np.min(ystm)), float(np.max(ystm)))

        # Note con LSB e check arrotondamenti entro LSB
        note_keras  = f"Baseline float32; LSB={out_scale:.6g}"
        note_tflite = f"Quantized on PC; LSB={out_scale:.6g}"
        if max_diff_ts is not None:
            note_stm32 = f"Dequantized output from MCU; LSB={out_scale:.6g}; Δmax_vs_TFLite={max_diff_ts:.6g}; entro_LSB={'YES' if entro_lsb else 'NO'}"
        else:
            note_stm32 = f"Dequantized output from MCU; LSB={out_scale:.6g}"

        header = ["Model", "MSE", "RMSE", "MAE", "MAX", "Range_out_min", "Range_out_max", "Notes"]
        rows = [
            ["Keras",     0.0,    0.0,    0.0,    0.0, keras_range[0],  keras_range[1],  note_keras],
            ["TFLite-PC", mse_t,  rmse_t, mae_t,  maxe_t, tflite_range[0], tflite_range[1], note_tflite],
            ["STM32",     mse_s,  rmse_s, mae_s,  maxe_s, stm32_range[0], stm32_range[1], note_stm32],
        ]
        ...
        print(f"[CSV] risultati salvati in {out_csv}")
