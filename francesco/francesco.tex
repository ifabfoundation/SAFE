...
\begin{figure}[!htbp]
    ...
    \includegraphics[
        angle=90,
        width=0.60\textheight,
        height=0.60\textwidth,
        keepaspectratio
    ]{assets/francesco/U545.jpg}
    \caption{U545RE}
\end{figure}
...
\section{Section 8 -- ESP32 Seco Board}
...
\begin{figure}[!htbp]
    ...
    \includegraphics[
        ...
        height=0.60\textwidth,
        keepaspectratio
    ]{assets/francesco/SECO.jpg}
    \caption{Seco Setup}
\end{figure}


\subsection{Installation}
First, Go on VSCode and install ESP32 IDF Estension, which suggest you to run this command:

\begin{center}
sudo apt-get install git wget flex ...
\end{center}
Save the new packages, if you want to delete them in the future:

\begin{center}
    bison ccache cmake cmake-data dfu-util flex gperf libffi-dev libfl-dev
    libfl2 libhiredis1.1.0 libjsoncpp25 librhash0 libssl-dev m4 ninja-build
    python3-venv
\end{center}
Fix the path for noble python3-venv if needed

\begin{center}
    sudo apt-get install --fix-missing
    sudo apt-get install python3-venv
\end{center}
There will be 2 new file/folder

\begin{center}
    /home/.../esp
    /home/.../.espressif
\end{center}
You can acess to the main page with New Proj | Import | Show examples... by clicking the extension in the left bar menu. The terminal here will use the esp/versionx/esp-idf/tools/idf.py executable. Let's read the SECO \textbf{board manual} of the EasyEdge. We see that it mount this chip:

\begin{center}
    \textit{ESP32-WROVER 3.3V}
\end{center}
So we have to create a new project using esp32 IDF target with this chip board and for now ttyUSB0 port (of the adapter). Before we get started, it would be necessary to explain the architecture scheme of this project:

\begin{itemize}
\item Sensor-side (USB-RS485-TTL) the pipeline is the same, a part of the mammut cable for male-male connectors.
\item DBG/FLASH-side it's not available an USB direct connection. So, we will have a dongle USB-TTL going on different GPIO as explained in the manual.
\end{itemize}
Now we can load a simple hello world project, keeping in mind the correct pipeline

\subsection{Deployment pipeline}

\subsubsection{Flashing Mode}

\begin{itemize}
\item Connect EasyEdge to PC with USB-TTL (P6) with these jumpers:
\end{itemize}
\begin{center}
JP3/JP8 \textbf{closed} → serial TTL | JP1 \textbf{closed} → bootloader | Others opened
\end{center}
\begin{itemize}
...
\end{center}
You will se the starting log. Now we can start converting the firmware from U545RE to ESP32.

\subsection{Firmware Porting}

\subsubsection{Important note}

\begin{itemize}
\item The python sending from the sensor dongle is the same, nothing to change
\item The Model has to be converted manually by using the C command:
\end{itemize}
\begin{center}
    \begin{verbatim}
...
\end{center}
\begin{itemize}
\item We will also have to implement the onboard standardization
\end{itemize}

\subsubsection{ESP32 Model deployment}

Let's add the TFLite Micro as a Component. Run this command on terminal:
\begin{center}
    idf.py add-dependency "esp-tflite-micro"
\end{center}
Now we can link tflite-micro. Then we show to the compiler the dependancy root on CMakeLists.txt(Yes, cpp and cc in the same component):
...
\end{lstlisting}
Entry point (main.c): initializes core/window manager, ML interpreter, and UART ingestion task, then idles while tasks run.

\begin{lstlisting}[language=C]
void app_main() {
    log("SAFE anomaly detection starting...");
    safe_core_init();
    safe_ml_init();
    safe_uart_init();
    while (true) delay(1s);
...
\end{lstlisting}
UART task streams host data into safe core; once window filled, it triggers ML inference.
...
\begin{lstlisting}
    safe_uart_init() {
        configure_uart0(115200);
        start_task(uart_rx_task);
    }
    
    uart_rx_task() {
        while (true) {
            bytes = uart_read(timeout=20ms);
            accumulate_line(bytes);
            if (line_complete) handle_line(line);
        }
    }
    
    handle_line(line) {
        values = parse_16_floats(line);
        full = safe_core_push_sample(values);
        uart_write("OK: riga accettata");
        if (full) safe_ml_run_inference();
    }
\end{lstlisting}
Maintaining the 30×16 sliding window, optional preprocessing, quantization/dequantization helpers, and anomaly logging for both INT8 and float models.

\begin{verbatim}
    safe_core.c:
\end{verbatim}
\begin{lstlisting}
    safe_core_init() { zero_window(); s_count = 0; s_ready = false; }
    
    safe_core_push_sample(sample[16]) {
        preprocessed = preprocess_sample(sample);
        slide_window(preprocessed);
        return s_ready;
    }
    
    safe_core_quantize_window_int8(dst) { copy_window_int8(dst, scale, zp); }
    safe_core_fill_window_float(dst)    { copy_window_float(dst); }
    
    safe_core_postprocess_and_log(int8_output) {
        max = dequant_and_find_max(int8_output);
        log_scores(max);
    }
    
    safe_core_postprocess_and_log_float(f32_output) {
        maybe_log_raw_output(f32_output);
    }
\end{lstlisting}
Converting the embedded TFLite model into a MicroInterpreter, registers all required ops, and delegates inference to the appropriate path based on tensor type.
...
\begin{lstlisting}
    safe_ml_init() {
        model = GetModel(model_int8_full_tflite);
        resolver = MicroMutableOpResolver<128>(AddAbs, AddConv2D, ...);
        interpreter = MicroInterpreter(model, resolver, tensor_arena);
        interpreter.AllocateTensors();
        s_input = interpreter->input(0);
        s_output = interpreter->output(0);
    }
    
    safe_ml_run_inference() {
        if (s_input->type == INT8) run_inference_int8();
        else if (s_input->type == FLOAT32) run_inference_float();
    }
    
    run_inference_int8() {
        safe_core_quantize_window_int8(s_input->data.int8);
        interpreter->Invoke();
        safe_core_postprocess_and_log(s_output->data.int8);
    }
    
    run_inference_float() {
        safe_core_fill_window_float(s_input->data.f);
        interpreter->Invoke();
        safe_core_postprocess_and_log_float(s_output->data.f);
    }
\end{lstlisting}
So exec start in main, inizializing all files. Then UART parse 16 values, push them into the sliding window and triggers inference if 30 is reached. So then the safe ml grabs the window and runs tflite microinterpreter and hands the output back to safe core. The result it's displayed.

\subsubsection{Flashing and debug}

Note: In order to flash \& debug while printing from .py, we follow the pipeline explained before:
Keeping JP3, JP8 always closed and the others opened, enter the flashing mode closing JP1, disconnecting sensor dongle (keeping only the debug/flash one, which name will be USB 0) pressing RESET and using:
\begin{center}
    idf.py -p /dev/ttyUSB0 build flash
\end{center}
(For check: ls /dev/ttyUSB*). Then, open JP1 and re-press RESET. We can now open the terminal:
\begin{center}
idf.py -p /dev/ttyUSB0 monitor
\end{center}
Now we can send on the gpio p4 using ttyusb1 on the .py script (dongle sensor).


\subsubsection{Simplification}

For an easier implementation, we will stay on a serial USB0 sending the window through the same port as the debug one. So we will write on TX and wait for ack on RX. At the end, a bigger wait for the inference. We will also use Float32 instead of int8. All these simplifications are made for a specific reason. We have to demonstrate that the model can run on the SECO ESP32 board as the U545RE. Of course the model is the same, and simply we can deploy the same modbus RTU with a different port for debug.

\subsubsection{Solved problems}

\begin{itemize}
\item Converter segfaults :The first TFLite conversions were crashing (segmentation fault) because we were trying to quantize aggressively with a complex SavedModel + representative dataset. We had to build a clean inference-only Keras model (batch=1), then convert with Optimize.DEFAULT and a lighter rep dataset.
\begin{lstlisting}[language=C]
    inference_input = tf.keras.Input(
        batch_shape=(1, WINDOW_SIZE, N_FEAT),
        name="input_layer_fixed_batch"
    )
\end{lstlisting}
\item Missing WHILE op on ESP32: switch to esp-tflite-micro that supports AddWhile() and actually register it.
\item Naive op resolver (too few ops). Initially we only added the “obvious” ops (Conv2D, MaxPool2D, Dense, LSTM, Quantize, Dequantize). Runtime then exploded with new missing ops (ExpandDims, Shape, StridedSlice, Pack, etc.). We ended up with a “kitchen sink” resolver using everything.
\end{itemize}

\subsection{Sending from sensor}
\begin{lstlisting}[language=Python]
# Serial and input configuration
PORT = "/dev/ttyUSB0"
BAUD = 115200
CSV  = "win_std.csv"

# Protocol tokens sent by the ESP32
ACK_TOKEN = "OK"
RUN_TOKEN = "safe_ml_run_inference"
RAW_END   = "RAW_OUT end"


def wait_ack(serial, timeout):
    # Wait for the ACK corresponding to a single transmitted line
    while not timeout_expired(timeout):
        for line in read_serial_lines(serial):
            if ACK_TOKEN in line:
                return True
    return False


def wait_inference(serial, timeout):
    # Wait until inference starts and raw output is completed
    ...
    while not timeout_expired(timeout):
        for line in read_serial_lines(serial):
            ...
            if inference_started and RAW_END in line:
                return True
    return False
...
def main():
    ...
    serial = open_serial(PORT, BAUD)

    # Load standardized samples from CSV
    lines = read_csv_lines(CSV)

    # Phase 1: send samples one by one, waiting for ACK each time
    for line in lines:
        serial.write(line)
        wait_ack(serial, timeout=1.5)

    # Phase 2: wait for ML inference and final output
    wait_inference(serial, timeout=10.0)
